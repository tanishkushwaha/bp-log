...
import { useFormData, FormDataType } from "@/contexts/FormDataContext";
...
export default function ReadingForm({ itemId }: { itemId?: string }) {
  ...
  useEffect(() => {
    ...
    if (selectedReading) {
      setFormData({
        sys: `${selectedReading.bp_sys}`,
        dia: `${selectedReading.bp_dia}`,
        pulse: `${selectedReading.pr}`,
        date: selectedReading.date,
        time: selectedReading.date,
      });
    } else {
      setFormData({
        sys: "",
